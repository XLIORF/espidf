...
void app_main(void)
{
    ...
    while(1)
    {
        ...
        {
            ESP_LOGI(TAG, "sent publish successful, msg_id=%d", esp_mqtt_client_publish(client, "led", "on", 0, 0, 0));
        }
        // read_button(BUTTON_RIGHT);
        if(read_button(BUTTON_DOWN) != 0)
        {
            ESP_LOGI(TAG, "sent publish successful, msg_id=%d", esp_mqtt_client_publish(client, "led", "off", 0, 0, 0));
        }
